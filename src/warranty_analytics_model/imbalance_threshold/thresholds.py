...
def pareto_frontier(curve: pd.DataFrame) -> list[dict[str, Any]]:
    """Return all nondominated precision/recall operating points.

    Recall is ordered from high to low so a point is retained whenever it
    improves precision over every point with at least as much recall. Equal
    recall rows are ordered by precision and then threshold for deterministic
    de-duplication.
    """

    ordered = curve.sort_values(
        ["recall", "precision", "threshold"], ascending=[False, False, True], kind="mergesort"
    )
