...
def _compare_numeric(left: float, right: float, *, higher_is_better: bool, tolerance: float) -> int:
...
def compare_phase12_candidates(
    left: dict[str, Any], right: dict[str, Any], *, tolerance: float = CHAMPION_TOLERANCE
) -> int:
    """Compare candidates using the locked tolerance-aware Phase 12 rule."""

    left_metrics = left["validation_metrics"]
    right_metrics = right["validation_metrics"]
    for metric, higher_is_better in (
        ...
        if comparison:
            return comparison
    for key in ("feature_count", "complexity_order"):
...
def select_phase12_champion(candidates: list[dict[str, Any]]) -> str:
    ...
    ordered = sorted(candidates, key=cmp_to_key(compare_phase12_candidates))
