...
def _compare_payload(expected: dict[str, Any], actual: Any, label: str, errors: list[str]) -> None:
    ...
    for key, expected_value in expected.items():
        ...
        if isinstance(expected_value, (int, float)) and not isinstance(expected_value, bool):
            if not _finite_equal(expected_value, actual_value):
    ...
    *,
    ...
    try:
        ...
        if not isinstance(declared_hashes, dict):
            raise ImbalanceThresholdError("Phase 12 artifact_file_sha256 is missing.")
        for name, digest in declared_hashes.items():
            artifact = directory / str(name)
            if not artifact.is_file() or sha256_file(artifact) != str(digest):
                raise ImbalanceThresholdError(f"Phase 12 artifact hash changed: {name}.")
        if not declared_hashes:
            warnings.append("ARTIFACT_HASHES_UNAVAILABLE_FOR_LEGACY_RUN")
        ...
        # Reconstruct the ranking and probability-distribution evidence directly
        # from OOF predictions.  The persisted fold table is an output, not a
        # source of truth for strategy selection.
        for (track, strategy_id, fold_id), group in oof.groupby(
            ["track", "strategy_id", "fold_id"], sort=True
        ):
            fold_row = fold_frame.loc[
                (fold_frame["track"] == track)
                & (fold_frame["strategy_id"] == strategy_id)
                & (fold_frame["fold_id"] == int(fold_id))
            ]
            if len(fold_row) != 1:
                errors.append(
                    f"Phase 12 fold metric row missing for {track}/{strategy_id}/{fold_id}."
                )
                continue
            fold = next(item for item in inputs.fold_plan.folds if item.fold_id == int(fold_id))
            ordered = group.sort_values(KEY, kind="mergesort")
            y = target_by_key.loc[ordered[KEY].tolist()].to_numpy(dtype="int8")
            probabilities = ordered["high_cost_probability"].to_numpy(dtype="float64")
            recomputed = ranking_metrics(y, probabilities)
            train_y = target_by_key.loc[list(fold.train_keys)].to_numpy(dtype="int8")
            recomputed.update(
                {
                    ...
                    "prediction_minimum": float(probabilities.min()),
                    "prediction_maximum": float(probabilities.max()),
                    "prediction_mean": float(probabilities.mean()),
                    "prediction_median": float(np.median(probabilities)),
                }
            )
            persisted_row = fold_row.iloc[0]
            for column in (
                "average_precision",
                "roc_auc",
                "log_loss",
                ...
                if not _finite_equal(persisted_row[column], recomputed[column]):
                    errors.append(
                        f"Phase 12 OOF-recomputed fold metric differs for {track}/{strategy_id}/{fold_id}: {column}."
                    )
            for column in (
                "train_positive_count",
                "train_negative_count",
                ...
                if int(persisted_row[column]) != int(recomputed[column]):
                    errors.append(
                        f"Phase 12 OOF-recomputed fold count differs for {track}/{strategy_id}/{fold_id}: {column}."
                    )
        ...
        if not isinstance(effective, list):
            raise ImbalanceThresholdError("Phase 12 effective model manifest schema changed.")
        model_by_candidate = {
            (str(entry["track"]), str(entry["candidate_id"])): entry for entry in model_entries
        }
        effective_by_track = {str(entry["track"]): entry for entry in effective}

        def recompute_validation_metrics(entry: dict[str, Any], threshold: float) -> dict[str, Any]:
            track = str(entry["track"])
            ...
            if rows.empty:
                raise ImbalanceThresholdError(
                    f"Phase 12 validation predictions missing for {candidate_id}."
                )
            y = validation_by_key.loc[rows[KEY].tolist()].to_numpy(dtype="int8")
            probabilities = rows["high_cost_probability"].to_numpy(dtype="float64")
            result = ranking_metrics(y, probabilities)
            result.update(threshold_metrics(y, probabilities, threshold))
            return result

        # Recompute Route A / Route B from the reproduced parent and weighted
        # validation predictions.  The persisted effective manifest is evidence
        # to compare against, never the decision source.
        for track in TRACKS:
            selected_id = str(manifest.get("selected_strategies", {}).get(track, ""))
            parent_id = inputs.parents[track].effective_candidate_id
            parent_entry = model_by_candidate.get((track, parent_id))
            effective_entry = effective_by_track.get(track)
            if parent_entry is None or effective_entry is None:
                errors.append(f"Phase 12 replacement evidence is incomplete for {track}.")
                continue
            parent_threshold = float(
                expected_thresholds[track]["S0_NONE"]["technical_default"]["threshold"]
            )
            parent_metrics = recompute_validation_metrics(parent_entry, parent_threshold)
            weighted_metrics: dict[str, Any] | None = None
            weighted_entry: dict[str, Any] | None = None
            if selected_id != "S0_NONE":
                weighted_id = f"P12_{track}_{selected_id}"
                weighted_entry = model_by_candidate.get((track, weighted_id))
                if weighted_entry is None:
                    errors.append(f"Phase 12 weighted replacement evidence is missing for {track}.")
                    continue
                weighted_threshold = float(
                    expected_thresholds[track][selected_id]["technical_default"]["threshold"]
                )
                weighted_metrics = recompute_validation_metrics(weighted_entry, weighted_threshold)
                expected_decision = replacement_decision(
                    parent_metrics,
                    weighted_metrics,
            ...
            replaced = bool(expected_decision["replace_parent"])
            expected_candidate = (
                str(weighted_entry["candidate_id"])
                if replaced and weighted_entry is not None
                else parent_id
            )
            expected_strategy = selected_id if replaced else "S0_NONE"
            expected_decision.update(
                {
                    "selected_strategy_id": selected_id,
                    "effective_candidate_id": expected_candidate,
                    "fallback_to_phase11_parent": not replaced,
                }
            )
            _compare_payload(
                expected_decision,
                effective_entry.get("replacement_decision"),
                f"Phase 12 replacement decision {track}",
                errors,
            )
            if effective_entry.get("candidate_id") != expected_candidate:
                errors.append(f"Phase 12 effective candidate changed for {track}.")
            if effective_entry.get("selected_imbalance_strategy") != expected_strategy:
                errors.append(f"Phase 12 effective strategy changed for {track}.")
            for candidate_entry, recomputed in (
                (parent_entry, parent_metrics),
                *(
                    [(weighted_entry, weighted_metrics)]
                    if weighted_entry and weighted_metrics
                    else []
                ),
            ):
                persisted_metrics = candidate_entry.get("validation_metrics", {})
                for key in recomputed:
                    if key in persisted_metrics and not _finite_equal(
                        recomputed[key], persisted_metrics[key]
                    ):
                        errors.append(
                            f"Phase 12 validation metric differs for {candidate_entry['candidate_id']}: {key}."
                        )
